		  ...
		  <div class="form-group menu-select" id="menu-component">
			...
			<select class="select-type form-control">
				...
				<xsl:for-each select="components">
						<option value="{id}"><xsl:value-of select="controller"/></option>
				</xsl:for-each>
			</select>
          </div>
		...
		<div class="form-group menu-select" id="menu-alias">
			...
			<select class="select-type form-control">
				...
				<xsl:for-each select="alias">
						<option value="{id}"><xsl:value-of select="alias"/></option>
				</xsl:for-each>
			</select>
          </div>
		...
		<div class="form-group menu-select" id="menu-content_listing">
			...
			<select class="select-type form-control">
				...
				<xsl:for-each select="lists">
						<option value="{id}"><xsl:value-of select="category"/></option>
				</xsl:for-each>
			</select>
          </div>
    ...
    <xsl:template name="naviEditable">
        <ol class="list-group dd-list">
            <xsl:for-each select="sub">
                <li data-id="{id}" data-name="{translation}" data-type="{type}" class="dd-item list-group-item" data-old="1">
						<xsl:attribute name="data-target"><xsl:choose><xsl:when test="type='url'"><xsl:value-of select="target"/></xsl:when><xsl:otherwise><xsl:value-of select="target_id"/></xsl:otherwise></xsl:choose></xsl:attribute>
